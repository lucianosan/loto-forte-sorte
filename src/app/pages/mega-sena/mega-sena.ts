...
export class MegaSena {
  ...
  availableNumbers: number[] = Array.from({length: 60}, (_, i) => i + 1);
  closureGuarantee = 4; // Padrão 4 (Quadra)
  ...
  get isCombinationValid(): boolean {
    return this.selectedNumbers.size >= 6 && this.selectedNumbers.size <= 15;
  }
  
  generateRandom() {
    this.generatedGames = this.gameService.generateRandomGame('MEGA_SENA', this.randomCount);
    this.currentPage = 1;
    this.clearResult();
  }
  ...
  exportPdf() {
    if (this.generatedGames.length > 0) {
      this.pdfService.generatePdf(this.generatedGames, 'MEGA_SENA', 'Mega Sena');
    }
  }
  ...
  generateCombinations() {
    if (this.isCombinationValid) {
      ...
      this.generatedGames = this.gameService.generateCombinations(numbers, 6);
      this.currentPage = 1;
      this.clearResult();
    }
  }
  ...
  generateClosure() {
    if (this.isCombinationValid) {
      const numbers = Array.from(this.selectedNumbers);
      this.generatedGames = this.gameService.generateClosure(numbers, 6, this.closureGuarantee);
      this.currentPage = 1;
      this.clearResult();
    }
  }
  ...
  checkResult() {
    ...
    const request = this.contestNumber 
      ? this.lotteryService.getResultByContest('MEGA_SENA', this.contestNumber)
      : this.lotteryService.getLatestResult('MEGA_SENA');
